...
Rule.prototype.getTableUI = function(allowImport, type) {
	...
	ui += 				'<a id="append-symbol-button-' + this.id + '" href="javascript:void(0)" class="btn btn-sm btn-default btn-flat pull-right margin-r-5"><i class="far fa-star margin-r-5"></i>' + gettext('Append symbolizer') + '</a>';
...
Rule.prototype.registerEvents = function(type) {
	...
	$("#edit-rule-" + this.id).on('click', function(e){
		...
		ui += 			'<i class="fa fa-save margin-r-5"></i>' + gettext('Save');
		ui += 		'</button>';
		ui += 	'</div>';
